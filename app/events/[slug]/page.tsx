...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}):Promise<Metadata>{ const {slug}=await params; const event=pastEvents.find(e=>e.slug===slug); return event?{title:event.title,description:event.description}:{title:"Event Not Found"}; }
export default async function EventPage({params}:{params:Promise<{slug:string}>}){const {slug}=await params;const event=pastEvents.find(e=>e.slug===slug);if(!event)notFound();return <main><HeroSection title={event.title} subtitle={`Home / Events / ${event.title}`} description={event.description} backgroundImage={event.image}/>
