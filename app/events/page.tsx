...
export default function EventsPage() { return <main>
  ...
  <section className={styles.section}><div className={styles.wrap}><div className={styles.sectionHead}><div><p className={styles.eyebrow}>Community journal</p><h2 className={styles.heading}>Moments that move<br />our story forward.</h2></div><p className={styles.lead}>Explore the events, competitions, and community days that have brought Ku-Ila together.</p></div><div className={styles.cards}>{pastEvents.map((event) => <Link className={styles.imageCard} href={`/events/${event.slug}`} key={event.id}><figure><Image src={event.image} alt={event.title} fill sizes="(max-width: 820px) 90vw, 30vw" /></figure><p className={styles.tag}>{event.date}</p><h3>{highlightLast(event.title)}</h3><p><MapPin size={13} style={{display:"inline", marginRight:5}} />{event.location}</p></Link>)}</div></div></section>
