...
export default function ProfileUpdateRequestModal({ member }: { member: MemberIdentity }) {
  ...
  useEffect(() => {
    ...
    setTimeout(() => dialog.current?.querySelector<HTMLElement>('input[name="full_name"]')?.focus(), 0);
    return () => {
  ...
  const advanceFocus = (event: React.FocusEvent<HTMLFormElement>) => {
    const field = event.target;
    if (!(field instanceof HTMLInputElement || field instanceof HTMLSelectElement) || field.type === "checkbox" || field.type === "file" || !field.value || !field.validity.valid) return;
    const fields = [...event.currentTarget.querySelectorAll<HTMLElement>('input:not([type="hidden"]):not([type="checkbox"]):not([type="file"]), select, textarea, button:not([disabled])')];
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    data.forEach((value, key) => {
      if (typeof value !== "string" || ["contact_email", "additional_information", "_honeypot"].includes(key)) return;
    ...
    const result = await submitContactForm({
      ...
      message: String(data.get("additional_information") || "Profile update request"),
      turnstileToken,
  ...
  const modal = open && typeof document !== "undefined" ? createPortal(<div className={styles.overlay} role="presentation"><div ref={dialog} className={styles.dialog} role="dialog" aria-modal="true" aria-labelledby={titleId}>
    ...
    {success ? <div className={styles.success}><Check size={34} /><h3>Thank you.</h3><p>Your profile update request has been received and will be reviewed before any changes are published.</p><button type="button" className={styles.submit} onClick={close}>Close</button></div> : <form onSubmit={submit} onBlurCapture={advanceFocus} className={styles.form} encType="multipart/form-data">
      ...
      <p className={styles.intro}>Submit the information you would like us to add or change. Your request is reviewed before it appears on your public profile.</p>
      <div className={styles.grid}><Field label="Full Name" required><input name="full_name" defaultValue={member.name} required placeholder="Ricson M. Cultura" /></Field><Field label="Contact Email" required help="Used only to reply about this request; it is not shown publicly."><input name="contact_email" type="email" required placeholder="name@example.com" /></Field><Field label="Nickname / Also Known As"><input name="nickname" placeholder="Rics" /></Field></div>
      <Field label="Would you like to request a name change?"><select name="name_change" onChange={event => setNameChange(event.target.value === "Yes")}><option>No</option><option>Yes</option></select></Field>{nameChange ? <><Field label="Requested Display Name"><input name="requested_display_name" placeholder="Ricson Cultura" /></Field><Field label="Reason for the requested name change"><textarea name="name_change_reason" placeholder="I would like my middle initial removed from the public profile." /></Field></> : null}
      <Field label="Community Affiliation" required help="Select your current affiliation with Ku-Ila."><select name="community_affiliation" required value={affiliation} onChange={event => setAffiliation(event.target.value)}><option value="">Choose an option</option><option>Member</option><option>Officer</option></select></Field>{affiliation === "Officer" ? <Field label="Designation" required><input name="designation" required placeholder="President, Secretary, or another officer role" /></Field> : null}
      <div className={styles.grid}><Field label="Organization / Chapter"><input name="chapter" placeholder="LGBTQIA++ SILBI Kumintang Ilaya" /></Field><Field label="Year Joined"><input name="year_joined" inputMode="numeric" pattern="[0-9]{4}" placeholder="2022" /></Field></div>
      <Field label="Currently Working As" help="Enter one occupation or profession per line."><textarea name="work" placeholder={"Makeup Artist\nFashion Designer"} /></Field><Field label="Skills and Talents" help="Enter one skill per line."><textarea name="skills" placeholder={"Hair and Makeup\nEvent Production\nPublic Speaking"} /></Field><div className={styles.grid}><Field label="Business, Brand, or Professional Name"><input name="business_name" placeholder="Ricson Cultura Makeup Artistry" /></Field><Field label="Website or Business Page"><input name="business_url" type="url" placeholder="https://example.com" /></Field></div>
      <Field label="Any information you'd like to add"><textarea name="additional_information" rows={5} placeholder="Share any other details you would like us to consider for your profile." /></Field>
      <Field label="Would you like to update your profile picture?"><select name="update_profile_photo" onChange={event => { const shouldUpdate = event.target.value === "Yes"; setUpdatePhoto(shouldUpdate); if (!shouldUpdate) setPhotoSelected(false); }}><option>No</option><option>Yes</option></select></Field>{updatePhoto ? <><Field label="Upload New Profile Photo" help="JPG, JPEG, PNG, or WebP, up to 5 MB. It may be cropped to match the member directory style."><input name="profile_photo" type="file" accept="image/jpeg,image/png,image/webp" onChange={event => setPhotoSelected(Boolean(event.target.files?.[0]))} /></Field>{photoSelected ? <label className={styles.agree}><input name="photo_permission" type="checkbox" required /> I confirm that I own this image or have permission to submit it for use on this website.</label> : null}</> : null}
      <div className={styles.grid}><Field label="Public Email Address"><input name="public_email" type="email" placeholder="name@example.com" /></Field><Field label="Contact Number"><input name="public_phone" type="tel" placeholder="+63 912 345 6789" /></Field></div><label className={styles.agree}><input name="email_permission" type="checkbox" /> I give permission for this email address to appear publicly.</label><label className={styles.agree}><input name="phone_permission" type="checkbox" /> I give permission for this phone number to appear publicly.</label><div className={styles.grid}>{["Facebook", "Instagram", "TikTok", "YouTube", "LinkedIn", "Other public profile"].map(network => <Field key={network} label={network}><input name={network.toLowerCase().replaceAll(" ", "_")} type="url" placeholder={`https://${network.toLowerCase().replaceAll(" ", "")}.com/username`} /></Field>)}</div>
      <div className={styles.review}><p>Your request is linked to <strong>{member.name}</strong> and will be reviewed before anything is published.</p><label className={styles.agree}><input name="accurate" type="checkbox" required /> I confirm that the information I submitted is accurate.</label><label className={styles.agree}><input name="review_understood" type="checkbox" required /> I understand that this request will be reviewed before it appears on the website.</label><label className={styles.agree}><input name="public_permission" type="checkbox" required /> I give permission for the approved information to appear on my public member profile.</label></div><div className={styles.turnstile}><TurnstileField onToken={setTurnstileToken} /></div>
      {error ? <p className={styles.error} role="alert">{error}</p> : null}<footer className={styles.footer}><button className={styles.submit} disabled={loading || (Boolean(process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY) && !turnstileToken)}>{loading ? <LoaderCircle className={styles.spin} size={16} /> : null} Submit Profile Update Request</button></footer>
    </form>}</div></div>, document.body) : null;

  return <><button ref={opener} type="button" className={styles.requestButton} onClick={() => { setSuccess(false); setNameChange(false); setAffiliation(""); setUpdatePhoto(false); setPhotoSelected(false); setOpen(true); }}><Pencil size={15} /> Request Profile Edit</button>{modal}</>;
}
