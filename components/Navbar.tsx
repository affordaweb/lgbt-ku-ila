        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-10">
          <div className="flex items-center gap-6">
            <a
              ...
              className="hover:text-[#f3702b] transition-colors"
            >
          ...
          <div className="flex items-center gap-6">
            <span className="flex items-center gap-1.5 text-gray-400">
              <MapPin className="w-3 h-3 text-[#f3702b]" />
              Kumintang Ilaya, Batangas City
            </span>
            <span className="flex items-center gap-1.5 text-gray-400">
              <Mail className="w-3 h-3 text-[#f3702b]" />
          ...
          <div className="flex items-center justify-between h-20">
            <Link href="/" className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-xs font-medium text-[#f3702b]">
                  Kumintang Ilaya
                </span>
              </div>
            </Link>
              ...
              {navLinks.map((link) => {
                ...
                return link.hasDropdown ? (
                  <div key={link.href} className="relative group">
                    <Link
                      href={link.href}
                      className="px-4 py-2 text-sm font-medium text-[#787878] hover:text-[#f3702b] transition-colors uppercase tracking-wider inline-flex items-center gap-1"
                    >
                      {link.label}
                      ...
                      <div className="py-2">
                        {dropdownItems?.map((item) => (
                          <Link
                            key={item.href}
                            href={item.href}
                            className="block px-5 py-3 text-sm text-[#787878] hover:text-[#f3702b] hover:bg-[#f7f7f7] transition-colors uppercase tracking-wider"
                          >
                            {item.label}
                          </Link>
                        ))}
                  ...
                  <Link
                    key={link.href}
                    href={link.href}
                    className="px-4 py-2 text-sm font-medium text-[#787878] hover:text-[#f3702b] transition-colors uppercase tracking-wider"
                  >
                    {link.label}
                  </Link>
                );
              ...
              return link.hasDropdown ? (
                <div key={link.href}>
                  <Link
                    ...
                    className="block px-4 py-3 text-sm font-medium text-[#787878] hover:text-[#f3702b] transition-colors uppercase tracking-wider"
                  >
                    {link.label}
                  </Link>
                  {dropdownItems?.map((item) => (
                    <Link
                      ...
                      href={item.href}
                      onClick={() => setIsOpen(false)}
                      className="block pl-8 pr-4 py-2 text-sm text-[#787878] hover:text-[#f3702b] transition-colors"
                    >
                      {item.label}
                    </Link>
                  ))}
